import type { DateTimeModification } from './event.ts';
...
export const supplementPolicyDescriptions: { [key: string]: string } = {
  ...
  MANDATORY_PERCENTAGE_RESERVATION:
    'Mandatory percentage fee, entire reservation (including user-selected Additional Items, if any)',
...
export function isMandatoryPercentage(supplementPolicy: SupplementPolicy): boolean {
  return (
    supplementPolicy === 'MANDATORY_PERCENTAGE_FOR_TICKET' || supplementPolicy === 'MANDATORY_PERCENTAGE_RESERVATION'
  );
}
