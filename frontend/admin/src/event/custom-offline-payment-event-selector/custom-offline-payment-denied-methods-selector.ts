...
export class CustomOfflinePaymentDeniedMethodsSelector extends LitElement {
    ...
    private _paymentMethodsTask = new Task(this, {
        task: async ([organization]) => {
            ...
            const deniedPaymentMethods =
                await this.paymentMethodService?.getDeniedPaymentMethodsForCategory(
                    this.event,
                    this.category,
                );
            ...
            return (
                orgPaymentMethods?.map((item) => ({
                    paymentMethod: item,
                    selected: deniedPaymentMethods?.includes(
    ...
    connectedCallback(): void {
        ...
        window.addEventListener(
            'update-payment-method-category-denied-list',
            this.handleUpdateDeniedListSubmit,
        );
    }
    ...
    handleUpdateDeniedListSubmit = async (_event: Event) => {
        ...
        if (!deniedPaymentMethodIds) return;
