...
export class CustomOfflinePaymentEventSelector extends LitElement {
    ...
    paymentMethodService?: CustomPaymentMethodsService =
        new CustomPaymentMethodsService();
    ...
    private _paymentMethodsTask = new Task(this, {
        task: async ([organization, event]) => {
            ...
            const orgPaymentMethods =
                await this.paymentMethodService?.getPaymentMethodsForOrganization(
                    organization,
                );
            ...
            if (event > -1) {
                const response =
                    await this.paymentMethodService?.getAllowedPaymentMethodsForEvent(
                        event,
                    );
                if (response) {
                    eventSelectedPaymentMethods = response.map(
            ...
            return (
                orgPaymentMethods?.map((item) => ({
                    paymentMethod: item,
                    selected: eventSelectedPaymentMethods.includes(
    ...
    connectedCallback(): void {
        ...
        window.addEventListener(
            'update-event-prices-form-submit',
            this.handleUpdatePricesModalSubmit,
        );
        window.addEventListener(
            'new-event-created',
            this.handleNewEventCreatedEvent,
        );
    }
    ...
    handleUpdatePricesModalSubmit = (_event: Event) => {
        ...
        const selectedIds = this.selectedPaymentMethods
            .filter(
                (method) =>
                    method.selected && method.paymentMethod.paymentMethodId,
            )
            .map((method) => method.paymentMethod.paymentMethodId);

        this.paymentMethodService?.setPaymentMethodsForEvent(
            eventId,
