...
export class AdditionalItemEdit extends LitElement {
                      ...
                      this.#form.field(
                        {
                          ...
                          validators: {
                            onChange: ({ value }) => {
                              return value && value.length < 3 ? 'error.title' : undefined;
                            },
                          },
                        },
                        (field) => {
                    ...
                    ${this.#form.field(
                      {
                        ...
                        validators: {
                          onChange: ({ value }) => {
                            return value && value.length < 3 ? 'error.description' : undefined;
                          },
                        },
                      },
                      (field) => {
  ...
  private renderAvailabilityAndPrices(formValue: AvailabilityAndPricesForm): TemplateResult {
    return html`
                    ...
                    ${this.#form.field(
                      {
                        ...
                        validators: {
                          onChange: ({ value }) => {
                            return value && value.length < 3 ? 'Not long enough' : undefined;
                          },
                        },
                      },
                      (field) => {
            ...
            ${renderIf(
              ...
              () => html`
                <div>
                    ${this.#form.field({ name: `availabilityAndPrices.fixPrice` }, (field) => {
                      return html`
            ...
            <div class="row" style="--alfio-row-cols: ${formValue.fixPrice ? '2' : '1'}">
                ${renderIf(
                  ...
                  () => html`
                    <div class="col">
                        ${this.#form.field({ name: `availabilityAndPrices.price` }, (field) => {
                          return html`
                ...
                `,
                    ...
                    <div class="col">
                        ${this.#form.field({ name: `availabilityAndPrices.vatType` }, (field) => {
                          return html`
    ...
    switch (formValue.supplementPolicy) {
      ...
      case 'MANDATORY_PERCENTAGE_FOR_TICKET':
        return html`
                    <div class="row">
                        <div class="col">
                            ${this.#form.field({ name: `availabilityAndPrices.minPrice` }, (field) => {
                              return html`
                        ...
                        <div class="col">
                            ${this.#form.field({ name: `availabilityAndPrices.maxPrice` }, (field) => {
                              return html`
  ...
  private addMaxQuantityPerOrder(formValue: AvailabilityAndPricesForm) {
    return html`
            ${this.#form.field({ name: `availabilityAndPrices.maxQtyPerOrder` }, (field) => {
              return html`
  ...
  private addAvailableQuantity() {
    if (this.event?.supportsAdditionalItemsQuantity) {
      return html`
                <div class="col">
                    ${this.#form.field({ name: `availabilityAndPrices.availableQuantity` }, (field) => {
                      return html`
      ...
      this.#form.api.update({
        ...
        validators: {
          onSubmitAsync: async (props) => {
            ...
            const additionalItemRequest: Partial<AdditionalItem> = this.buildServerPayload(
              props.value,
              request.ordinal,
            );
  ...
  private buildServerPayload(value: FormData, ordinal: number): Partial<AdditionalItem> {
    return {
      ...
      title: value.descriptions.map((df) => {
        return { locale: df.locale, type: 'TITLE', value: df.title, id: df.titleId };
      }),
      description: value.descriptions.map((df) => {
        return { locale: df.locale, type: 'DESCRIPTION', value: df.description, id: df.descriptionId };
      }),
