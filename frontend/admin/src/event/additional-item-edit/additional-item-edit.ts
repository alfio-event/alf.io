import type { SlDialog, SlRequestCloseEvent } from '@shoelace-style/shoelace';
...
export class AdditionalItemEdit extends LitElement {
    ...
    private buildDefaultValues(currentState: { values: FormData }): FormData {
        ...
        return {
            availabilityAndPrices:
                this.event != null
                    ? this.buildAvailabilityAndPricesFromEvent(this.event)
                    : currentState.values.availabilityAndPrices,
    ...
    private buildAvailabilityAndPricesFromEvent(
        event: AlfioEvent,
    ): AvailabilityAndPricesForm {
        const now = new Date();
        return {
            availableItems: 0,
            minPrice: null,
    ...
    private buildAvailabilityAndPricesFromItem(
        item: AdditionalItem,
    ): AvailabilityAndPricesForm {
        return {
            availableItems:
                item.availableItems !== -1 ? item.availableItems : null,
    ...
    private renderAvailabilityAndPrices(
        formValue: AvailabilityAndPricesForm,
    ): TemplateResult {
        return html`
            ${renderIf(
                ...
                <div>
                    ${this.#form.field(
                        { name: `availabilityAndPrices.supplementPolicy` },
                        (field) => {
                            return html`
            ...
            <div class="row">
                <div class="col">
                    ${this.#form.field(
                        {
                            name: `availabilityAndPrices.inception`,
                            validators: {
                                onChange: ({ value }) => {
                                    return value && value.length < 3
                                        ? 'Not long enough'
                ...
                <div class="col">
                    ${this.#form.field(
                        {
                            name: `availabilityAndPrices.expiration`,
                            validators: {
                                onChange: ({ value }) => {
                                    return value && value.length < 3
                                        ? 'Not long enough'
            ...
            ${renderIf(
                ...
                <div>
                    ${this.#form.field(
                        { name: `availabilityAndPrices.fixPrice` },
                        (field) => {
                            return html`
            ...
            <div class="row" style="--alfio-row-cols: ${formValue.fixPrice ? '2' : '1'}">
                ${renderIf(
                    ...
                    <div class="col">
                        ${this.#form.field(
                            { name: `availabilityAndPrices.price` },
                            (field) => {
                                return html`
    ...
    private handleContextBasedFields(
        formValue: AvailabilityAndPricesForm,
    ): TemplateResult {
            ...
            case 'MANDATORY_PERCENTAGE_FOR_TICKET':
                return html`
                    <div class="row">
                        <div class="col">
                            ${this.#form.field(
                                { name: `availabilityAndPrices.minPrice` },
                                (field) => {
                                    return html`
                        ...
                        <div class="col">
                            ${this.#form.field(
                                { name: `availabilityAndPrices.maxPrice` },
                                (field) => {
                                    return html`
    ...
    private addMaxQuantityPerOrder(formValue: AvailabilityAndPricesForm) {
        return html`
            ${this.#form.field(
                { name: `availabilityAndPrices.maxQtyPerOrder` },
                (field) => {
                    return html`
    ...
    private addAvailableQuantity() {
        if (this.event?.supportsAdditionalItemsQuantity) {
            return html`
                <div class="col">
                    ${this.#form.field(
                        { name: `availabilityAndPrices.availableQuantity` },
                        (field) => {
                            return html`
                ...
                validators: {
                    onSubmitAsync: async (props) => {
                        ...
                        const additionalItemRequest: Partial<AdditionalItem> =
                            this.buildServerPayload(
                                props.value,
                                request.ordinal,
                            );
                        const result =
                            await AdditionalItemService.validateAdditionalItem(
    ...
    private buildServerPayload(
        value: FormData,
        ordinal: number,
    ): Partial<AdditionalItem> {
        return {
            id: this.editedItem?.id,
            maxQtyPerOrder: value.availabilityAndPrices.maxQtyPerOrder ?? -1,
            price: value.availabilityAndPrices.price,
            availableQuantity:
                value.availabilityAndPrices.availableQuantity ?? -1,
            inception: toDateTimeModification(
                value.availabilityAndPrices.inception,
            ),
            expiration: toDateTimeModification(
                value.availabilityAndPrices.expiration,
            ),
            title: value.descriptions.map((df) => {
                return {
                    locale: df.locale,
                    type: 'TITLE',
                    value: df.title,
            ...
            description: value.descriptions.map((df) => {
                return {
                    locale: df.locale,
                    type: 'DESCRIPTION',
    ...
    private async save(value: FormData, ordinal: number) {
        const additionalItemRequest: Partial<AdditionalItem> =
            this.buildServerPayload(value, ordinal);
        const update = await AdditionalItemService.updateAdditionalItem(
            additionalItemRequest,
            this.event!.id,
        );
        if (update.ok) {
    ...
    private validationError(fieldName: string): string | undefined {
        ...
        if (validationErrors != null) {
            const remoteFieldName = fieldName.substring(
