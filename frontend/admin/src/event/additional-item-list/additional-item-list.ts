...
import { AdditionalItemService, type UsageCount } from '../../service/additional-item.ts';
...
export class AdditionalItemList extends LitElement {
        ...
        async (_) => {
          ...
          return {
            ...
            allowDownload: Object.values(count).some(
              (p) => Object.values(p).reduce((pv: number, cv: number) => pv + cv, 0) > 0,
            ),
          };
        },
        () => [this.refreshCount],
                            ...
                            <div class="info-container">
                                ...
                                ${renderIf(
                                  () =>
                                    item.fixPrice &&
                                    (item.type === 'DONATION' ||
                                      (!isMandatory(item.supplementPolicy) &&
                                        item.supplementPolicy !== 'OPTIONAL_UNLIMITED_AMOUNT')),
                                  () => html`
                                    <div class="info">
  ...
  private renderMoveButtons(listData: ListData, index: number, item: AdditionalItem, model: Model) {
    return renderIf(
      () => model.event.supportsAdditionalServicesOrdinal && listData.items.length > 1,
      () => html`
            <div class="button-container">
                ${renderIf(
