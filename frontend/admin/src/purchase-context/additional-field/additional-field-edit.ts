import type { SlDialog, SlRequestCloseEvent, SlSelect } from '@shoelace-style/shoelace';
...
import type { ContentLanguage, PurchaseContext } from '../../model/purchase-context.ts';
...
export class AdditionalFieldEdit extends LitElement {
        ...
        const selectionChangeHandler = (e: InputEvent) => {
          ...
          if (supportsRestrictedValues(value) && this.#form.api.getFieldValue('selectableOptions').length === 0) {
            ...
            this.#form.api.setFieldValue('selectableOptions', [
              { fieldName: '', description: {}, toBePersisted: true },
            ]);
          }
                      ...
                      (selectableOptionsField) => {
                        ...
                        return html`
                            ...
                            <sl-button class="mt-2" variant="success" @click=${() => {
                              selectableOptionsField.pushValue({ fieldName: '', description: {}, toBePersisted: true });
      ...
      if (languageIdx === 0 && !this.editField) {
        ...
        if (currentFieldNameTouched || currentFieldName?.length === 0) {
          this.#form.api.setFieldValue(
            `selectableOptions[${optionIndex}].fieldName`,
            value.toLowerCase().replaceAll(/\W/g, '_').substring(0, 64),
            { dontUpdateMeta: true },
          );
        }
      }
    ...
    return html`
            <sl-card class="item bg-default mt-3">
                <div slot="header">
                    ...
                    <div class="col">
                        ${renderIf(
                          () => this.#form.api.getFieldValue(`selectableOptions[${optionIndex}].toBePersisted`),
                          () =>
  ...
  private renderCategories(
    event: AlfioEvent,
    field: { state: { meta: any; value?: number[] }; handleChange: (values: any) => void },
  ) {
    const selectionChanged = (e: InputEvent) => {
  ...
  private renderAdditionalItemSelector() {
    ...
    return html`
            <li class="list-group-item">
                ...
                ${renderIf(
                  () => this.#form.api.state.values.context === 'ADDITIONAL_SERVICE',
                  () => html`
                    ${this.#form.field(
                      {
    ...
    if (this.dialog != null) {
      ...
      if (request.purchaseContext.type === 'event' && !this.editField) {
        this.additionalItems.push(
          ...(await AdditionalItemService.loadAll({ eventId: (request.purchaseContext as AlfioEvent).id })),
        );
      }
