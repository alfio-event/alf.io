...
import type { AdditionalField, AdditionalFieldStats } from '../../model/additional-field.ts';
...
export class AdditionalFieldStatistics extends LitElement {
  ...
  private readonly retrievePageDataTask = new Task<ReadonlyArray<boolean>, ReadonlyArray<AdditionalFieldStats>>(
    this,
    async () => {
    ...
    return html`
            <sl-drawer label=${`Statistics for ${this.field?.name}`} id="drawer-statistics">
               ${renderIf(
                 () => this.active,
                 () =>
                   this.retrievePageDataTask.render({
  ...
  public async show(request: { purchaseContext: PurchaseContext; field: AdditionalField }) {
    if (this.drawer != null) {
