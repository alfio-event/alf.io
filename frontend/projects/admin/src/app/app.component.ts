...
export class AppComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.organizationId$ = this.router.events.pipe(
      filter((a) => a instanceof ActivationEnd),
      map((a) => {
    ...
    this.currentOrganization$ = combineLatest([
      this.organizationId$,
      this.organizations$,
    ]).pipe(
      map(([id, orgs]) =>
        orgs.find((o) => id !== null && o.id === Number.parseInt(id))
      )
    );
  }
  ...
  public openOrgSelector(): void {
    ...
    modalRef.result
      .then((res: Organization) => {
        this.router.navigate(['/organization', res.id, 'event']).then((r) => {
          if (r) {
            this.currentOrganization$ = of(res);
          }
        });
      })
      .catch(() => {
