...
export class AdditionalFieldService {
    ...
    private static getUserDataLabelValue(
        name: string,
        index: number,
        userLanguage?: string,
        additionalData?: UserAdditionalData,
    ): { l: string; v: string } | null {
        if (
            additionalData != null &&
            additionalData[name] &&
            additionalData[name].values.length > index
        ) {
            const val = additionalData[name];
            return {
                l: val.label[userLanguage] || val.label[0],
                v: val.values[index],
            };
        }
    ...
    public buildAdditionalFields(
        before: AdditionalField[],
        after: AdditionalField[],
        userLanguage: string,
        userData?: UserAdditionalData,
    ): FormGroup {
        const additional = {};
        if (before) {
            this.buildSingleAdditionalField(
                before,
                additional,
                userLanguage,
                userData,
    ...
    private buildSingleAdditionalField(
        a: AdditionalField[],
        additional: {},
        userLanguage: string,
        userData?: UserAdditionalData,
        ...
        a.forEach((f) => {
            ...
            if (f.type === 'checkbox') {
                // pre-fill with empty values for the checkbox cases, as we can have multiple values!
            ...
            f.fields.forEach((field) => {
                arr[field.fieldIndex] = this.formBuilder.control(
                    field.fieldValue ||
                        AdditionalFieldService.getUserDataLabelValue(
                            f.name,
                            field.fieldIndex,
                            userLanguage,
                            userData,
