...
import type { CustomOfflinePayment } from "../model/event";
import type { DynamicDiscount } from "../model/event-code";
import type { OverviewConfirmation } from "../model/overview-confirmation";
import type { TransactionInitializationToken } from "../model/payment";
import type {
  ReservationInfo,
  ReservationStatusInfo,
} from "../model/reservation-info";
import type { ReservationPaymentResult } from "../model/reservation-payment-result";
...
export class ReservationService {
  ...
  reserveTickets(
    ...
    lang: string,
  ): Observable<ValidatedResponse<string>> {
  ...
  getReservationInfo(reservationId: string): Observable<ReservationInfo> {
    return this.http.get<ReservationInfo>(
      `/api/v2/public/reservation/${reservationId}`,
    );
  }

  getReservationStatusInfo(
    reservationId: string,
  ): Observable<ReservationStatusInfo> {
    return this.http.get<ReservationStatusInfo>(
      `/api/v2/public/reservation/${reservationId}/status`,
    );
  }
  ...
  validateToOverview(
    reservationId: string,
    contactsAndTicket: any,
    lang: string,
    ignoreWarnings: boolean,
  ): Observable<ValidatedResponse<boolean>> {
    const url = `/api/v2/public/reservation/${reservationId}/validate-to-overview`;
    return this.http.post<ValidatedResponse<boolean>>(url, contactsAndTicket, {
      params: { lang: lang, ignoreWarnings: "" + ignoreWarnings },
    });
  }

  confirmOverview(
    reservationId: string,
    overviewForm: OverviewConfirmation,
    lang: string,
  ): Observable<ValidatedResponse<ReservationPaymentResult>> {
    const url = `/api/v2/public/reservation/${reservationId}`;
    return this.http.post<ValidatedResponse<ReservationPaymentResult>>(
      url,
      overviewForm,
      { params: { lang: lang } },
    );
  ...
  initPayment(
    reservationId: string,
  ): Observable<TransactionInitializationToken> {
    return this.http.post<TransactionInitializationToken>(
  ...
  getPaymentStatus(
    reservationId: string,
  ): Observable<ReservationPaymentResult> {
    return this.http.get<ReservationPaymentResult>(
      `/api/v2/public/reservation/${reservationId}/payment/CREDIT_CARD/status`,
    );
  }

  forcePaymentStatusCheck(
    reservationId: string,
  ): Observable<ReservationPaymentResult> {
    return this.http.get<ReservationPaymentResult>(
  ...
  checkDynamicDiscountAvailability(
    eventShortName: string,
    reservation: ReservationRequest,
  ): Observable<DynamicDiscount> {
    return this.http.post<DynamicDiscount>(
  ...
  getApplicableCustomPaymentMethodDetails(reservationId: string) {
    return this.http.get<CustomOfflinePayment[]>(
