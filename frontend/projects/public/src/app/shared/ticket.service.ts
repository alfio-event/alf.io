...
export class TicketService {
  ...
  getTicketInfo(
    ...
    return this.http.get<TicketInfo>(
      `/api/v2/public/event/${eventName}/ticket/${ticketIdentifier}`,
    );
  }
  ...
  getTicket(
    eventName: string,
    ticketIdentifier: string,
  ): Observable<TicketsByTicketCategory> {
    return this.http.get<TicketsByTicketCategory>(
      `/api/v2/public/event/${eventName}/ticket/${ticketIdentifier}/full`,
    );
  }

  getOnlineCheckInInfo(
    eventName: string,
    ticketIdentifier: string,
    checkInCode: string,
  ): Observable<DateValidity> {
    return this.http.get<DateValidity>(
      `/api/v2/public/event/${eventName}/ticket/${ticketIdentifier}/code/${checkInCode}/check-in-info`,
      {
        params: {
  ...
  sendTicketByEmail(
    ...
    return this.http.post<boolean>(
      `/api/v2/public/event/${eventName}/ticket/${ticketIdentifier}/send-ticket-by-email`,
      {},
    );
  }

  buildFormGroupForTicket(
    ticket: Ticket,
    user?: User,
    additionalServicesWithData?: AdditionalServiceWithData[],
  ): UntypedFormGroup {
    return this.formBuilder.group(
      this.buildTicket(ticket, user, additionalServicesWithData),
    );
  }

  buildAdditionalServicesFormGroup(
    ...
    additionalServices.forEach((asw) => {
      ...
      byTicketUuid[asw.ticketUUID].push(
        this.buildAdditionalServiceGroup(asw, lang),
      );
    });
  ...
  updateTicket(
    ...
    return this.http.put<ValidatedResponse<boolean>>(
      `/api/v2/public/event/${eventName}/ticket/${ticketIdentifier}`,
      ticket,
    );
  }
  ...
  openReleaseTicket(ticket: Ticket, eventName: string): Observable<boolean> {
    return from(
      this.modalService.open(ReleaseTicketComponent, { centered: true }).result,
    ).pipe(
      mergeMap((res) => {
        if (res === "yes") {
  ...
  openDownloadTicket(
    ...
    const modal = this.modalService.open(DownloadTicketComponent, {
      centered: true,
    });
    ...
    return from(modal.result).pipe(map((_) => true));
  }
  ...
  private buildTicket(
    ...
    return {
      ...
      additionalServices: this.buildAdditionalServicesFormGroup(
