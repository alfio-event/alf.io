...
export class InvalidFeedbackDirective implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    @Optional() private control: FormControlName,
  ...
  ngOnInit(): void {
    if (
      this.control == null &&
      this.invalidFeedbackForm != null &&
      this.invalidFeedbackFieldName != null
    ) {
      this.targetControl = this.invalidFeedbackForm.get([
        this.invalidFeedbackFieldName,
      ]);
  ...
  private checkValidation(): void {
    ...
    if (this.invalidFeedbackInLabel) {
      errorContainerElement =
        this.element.nativeElement.parentElement.nextElementSibling;
    } else {
    ...
    if (
      ...
      this.errors.serverError.length > 0
    ) {
      ...
      if (isInvalidFeedbackContainer(errorContainerElement)) {
        ...
        if (this.invalidFeedbackInLabel) {
          ...
          this.element.nativeElement.parentNode.insertAdjacentElement(
            "afterEnd",
            container,
          );
        } else {
          this.element.nativeElement.insertAdjacentElement(
            "afterEnd",
            container,
          );
        }
  ...
  private addErrorMessages(container: HTMLElement): void {
    this.errors.serverError.forEach((e: ErrorDescriptor) => {
      ...
      this.subs.push(
        this.translation.stream(e.code, e.arguments).subscribe((text) => {
