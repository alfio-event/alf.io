...
import type { AnalyticsConfiguration } from '../model/analytics-configuration';
...
export class AnalyticsService {
    private gaScript: Observable<
        [Function, AnalyticsConfiguration, string]
    > | null = null;
    ...
    private handleGoogleAnalytics(
        conf: AnalyticsConfiguration,
        locationPathName: string,
    ) {
        if (this.gaScript === null) {
            this.gaScript = new Observable<
                [Function, AnalyticsConfiguration, string]
            >((subscribe) => {
                ...
                } else if (script.getAttribute(initAttribute) == null) {
                    // <- script has been created, but not loaded
                    subscribe.next([
                        this.initGtag(conf, script),
                        conf,
                        locationPathName,
                    ]);
                } else {
