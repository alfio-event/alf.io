...
export class LanguageSelectorComponent implements OnInit, OnChanges {
  ...
  private buildValues(): void {
    if (this.contentLanguages != null && this.contentLanguages.length > 0) {
      let currentLang = this.contentLanguages.find(
        (cl) => cl.locale === this.selectedLanguage,
      );
      ...
      this.currentLanguage = currentLang
        ? currentLang.displayLanguage
        : this.contentLanguages[0].displayLanguage;
      this.filteredLanguages = this.contentLanguages
        .filter((cl) => cl !== currentLang)
        .sort((a, b) =>
          a.displayLanguage.toLowerCase() > b.displayLanguage.toLowerCase()
            ? 1
            : -1,
        );
    }
  ...
  public changeLanguage(lang: string): void {
    ...
    const eventShortName = this.router.routerState.snapshot.root.firstChild
    ...
    if (eventShortName != null) {
      observable = this.i18nService.useTranslation(
        "event",
        eventShortName,
        lang,
      );
