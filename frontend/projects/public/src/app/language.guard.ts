...
export class LanguageGuard {
    ...
    canActivate(
        ...
        const publicIdentifier =
            next.params[next.data.publicIdentifierParameter];
        const req =
        ...
        return req.pipe(
            switchMap((availableLanguages) => {
                const lang = this.extractLang(
                    availableLanguages,
                    persisted,
                    langQueryParam,
                );
                return this.i18nService.useTranslation(
                    type,
                    publicIdentifier,
                    lang,
                );
    ...
    private getForContext(
        type: PurchaseContextType,
        publicIdentifier: string,
    ): Observable<string[]> {
        return this.purchaseContextService
            .getContext(type, publicIdentifier)
            .pipe(map((p) => p.contentLanguages.map((v) => v.locale)))
            .pipe(catchError((val) => this.getForApp()));
    }

    private getForApp(): Observable<string[]> {
        return this.i18nService
            .getAvailableLanguages()
            .pipe(map((languages) => languages.map((l) => l.locale)));
    }
    ...
    private extractLang(
        ...
        if (override && availableLanguages.indexOf(override) >= 0) {
            ...
            availableLanguages.indexOf(this.translate.getBrowserLang()) >= 0
        ) {
