...
export class DOMXsrfTokenExtractor implements HttpXsrfTokenExtractor {
    ...
    getToken(): string | null {
        ...
        return (
            this.doc.head.querySelector<HTMLMetaElement>(
...
export class DOMGidExtractor {
    ...
    getToken(): string | null {
        ...
        return (
            this.doc.head.querySelector<HTMLMetaElement>('meta[name="GID"]')
                ?.content ?? null
        );
    }
...
function retrieveHeadElement(name: string, doc: Document): HTMLMetaElement {
    let element = doc.head.querySelector<HTMLMetaElement>(
        `meta[name="${name}"]`,
    );
    if (element == null) {
...
export class AuthTokenInterceptor implements HttpInterceptor {
                ...
                tap((response) => {
                    if (response instanceof HttpResponse) {
                        if (response.headers.has(GID_HEADER_NAME)) {
                            this.domGidExtractor?.updateToken(
                                response.headers.get(GID_HEADER_NAME),
                            );
                        }
                        if (
                            req.method === 'GET' &&
                            response.headers.has(XSRF_TOKEN_HEADER)
                        ) {
                            this.domXsrfTokenExtractor?.updateToken(
                                response.headers.get(XSRF_TOKEN_HEADER),
                            );
                        }
                    }
                }),
    ...
    private init(): void {
        if (this.domGidExtractor == null) {
            ...
            this.domXsrfTokenExtractor = this.injector.get(
