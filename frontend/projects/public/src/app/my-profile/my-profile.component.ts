...
export class MyProfileComponent implements OnInit {
  ...
  ngOnInit(): void {
    zip(
      this.userService.getUserIdentity(),
      ...
      if (additionalData != null) {
        const additionalInfoGroup = this.userForm.get(
          "additionalInfo",
        ) as UntypedFormGroup;
        for (const additionalOptionKey of Object.keys(additionalData)) {
          additionalInfoGroup.addControl(
            additionalOptionKey,
            this.formBuilder.control(
              additionalData[additionalOptionKey].values[0],
            ),
          );
        }
      }
      ...
      next: (res) => {
        if (res.success) {
          this.feedbackService.showSuccess(
            this.translateService.instant("my-profile.update.success"),
          );
          this.user = res.value;
        } else {
  ...
  deleteProfile(): void {
    const modalRef = this.modalService.open(MyProfileDeleteWarningComponent, {
      centered: true,
      backdrop: "static",
    });
  ...
  get hasAdditionalData(): boolean {
    return (
      this.additionalData != null && Object.keys(this.additionalData).length > 0
    );
  }
}
