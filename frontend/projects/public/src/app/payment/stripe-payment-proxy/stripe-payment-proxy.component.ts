...
import {
    STRIPE_CHECKOUT_ID_SCRIPT,
    StripeCheckoutPaymentProvider,
    StripePaymentV3,
} from './stripe-payment-providers';
...
export class StripePaymentProxyComponent implements OnChanges, OnDestroy {
    ...
    private unloadAll(): void {
        const checkoutScript = document.getElementById(
            STRIPE_CHECKOUT_ID_SCRIPT,
        );
    ...
    private loadSCA(): void {
        if (!document.getElementById(STRIPE_V3_ID_SCRIPT)) {
            ...
            document
                .getElementById(STRIPE_V3_ID_SCRIPT)
    ...
    private configureSCA() {
        ...
        if (this.parameters['stripeConnectedAccount']) {
            options['stripeAccount'] =
                this.parameters['stripeConnectedAccount'];
        }

        const stripeHandler = Stripe(this.parameters['stripe_p_key'], options);
        const card = stripeHandler
            .elements({ locale: this.translate.currentLang })
            .create('card', {
                style: {
                    base: {
                        color: '#495057',
