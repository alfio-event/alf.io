...
export class EventDisplayComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        ...
        this.route.params.subscribe((params) => {
            ...
            zip(
                ...
                if (code) {
                    this.internalApplyPromoCode(
                        code,
                        (err) => (this.globalErrors = err),
                    );
                }
            });
        });
    ...
    private internalApplyPromoCode(
        ...
        this.eventService
            .validateCode(this.event.shortName, promoCode)
            .subscribe(
                (res) => {
                    if (res.success) {
    ...
    ticketsLeftCountVisibleForCategory(category: TicketCategory): boolean {
        return (
            category.availableTickets != null && category.availableTickets > 0
        );
    }
    ...
    get dynamicDiscountMessage(): string {
        if (this.dynamicDiscount != null) {
            return this.dynamicDiscount.formattedMessage[
