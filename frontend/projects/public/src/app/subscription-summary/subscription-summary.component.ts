...
import type { SubscriptionSummaryData } from "../model/subscription";
...
export class SubscriptionSummaryComponent {
  ...
  get validFrom(): string {
    if (this.subscription.formattedValidFrom != null) {
      return getLocalizedContent(
        this.subscription.formattedValidFrom,
  ...
  get validTo(): string {
    if (this.hasValidTo) {
      return getLocalizedContent(
        this.subscription.formattedValidTo,
        this.translateService.currentLang,
      );
    }
    return "";
  ...
  getValue(field: AdditionalField): string {
    if (field.type === "input:dateOfBirth") {
      return this.dateFormat.transform(
        field.value,
        this.translateService.instant("common.date-format"),
      );
    }
    return (
      field.description[this.translateService.currentLang]
