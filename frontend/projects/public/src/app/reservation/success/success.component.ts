...
export class SuccessComponent implements OnInit {
    ...
    public ngOnInit(): void {
        this.route.params.subscribe((params) => {
            ...
            this.infoService
                ...
                .subscribe(
                    (info) =>
                        (this.walletConfiguration = info.walletConfiguration),
                );
            ...
            .subscribe((res) => {
                ...
                if (!this.reservationFinalized) {
                    pollReservationStatus(
                        this.reservationId,
                        this.reservationService,
                        (res1) => this.processReservationInfo(res1),
                    );
                }
            });
    }
    ...
    private processReservationInfo(res: ReservationInfo) {
        ...
        res.ticketsByCategory.forEach((tc) => {
            tc.tickets.forEach((ticket: Ticket) => {
                ...
                this.ticketsAllAssigned =
                    this.ticketsAllAssigned && ticket.assigned;
            });
        });
    }

    private buildFormControl(ticket: Ticket): void {
        this.ticketsFormControl[ticket.uuid] =
            this.ticketService.buildFormGroupForTicket(
                ticket,
                null,
                this.additionalServicesWithData[ticket.uuid],
            );
    }

    sendEmailForTicket(ticketIdentifier: string): void {
        this.ticketService
            .sendTicketByEmail(this.eventShortName, ticketIdentifier)
            .subscribe((res) => {
                if (res) {
    ...
    updateTicket(uuid: string): void {
        ...
        this.ticketService
            .updateTicket(this.event.shortName, uuid, ticketValue)
            .subscribe(
                (res) => {
                    if (res.success) {
                        this.loadReservation();
    ...
    releaseTicket(ticket: Ticket) {
        this.ticketService
            .openReleaseTicket(ticket, this.event.shortName)
            .subscribe((released) => {
                if (released) {
                    const singleTicket =
                        this.reservationInfo.ticketsByCategory
                            .map((c) => c.tickets.length)
                            .reduce((c1, c2) => c1 + c2) === 1;
                    if (singleTicket) {
    ...
    public isOnlineTicket(category: TicketsByTicketCategory): boolean {
        return (
            this.event.format === 'ONLINE' ||
            (this.event.format === 'HYBRID' &&
                category.ticketAccessType === 'ONLINE')
        );
    }
    ...
    getAdditionalDataForm(ticket: Ticket): UntypedFormArray | null {
        const linksGroup = <UntypedFormGroup>(
            this.ticketsFormControl[ticket.uuid].get('additionalServices')
        );
        return linksGroup.contains(ticket.uuid)
            ? <UntypedFormArray>linksGroup.get(ticket.uuid)
            : null;
    }
    ...
    getGroupedAdditionalData(
        ticket: Ticket,
    ): GroupedAdditionalServiceWithData[] {
        return groupAdditionalData(
            this.additionalServicesWithData[ticket.uuid] ?? [],
        );
    }
}
