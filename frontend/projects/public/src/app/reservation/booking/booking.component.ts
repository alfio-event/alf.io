...
export class BookingComponent implements OnInit, AfterViewInit {
    ...
    zip(
      ...
      zip(
        ...
        next: ([purchaseContext, reservationInfo]) => {
          ...
          const invoiceRequested = purchaseContext.invoicingConfiguration
            .onlyInvoice
            ? true
          ...
          this.contactAndTicketsForm = this.formBuilder.group({
            ...
            billingAddressCompany:
              billingDetails.companyName || userBillingDetails?.companyName,
            billingAddressLine1:
              billingDetails.addressLine1 || userBillingDetails?.addressLine1,
            billingAddressLine2:
              billingDetails.addressLine2 || userBillingDetails?.addressLine2,
            ...
            vatCountryCode:
              billingDetails.country || userBillingDetails?.country,
            ...
            italyEInvoicingReferenceType: BookingComponent.optionalGet(
              billingDetails,
              (i) => i.referenceType,
              userBillingDetails,
            ),
            italyEInvoicingReferenceAddresseeCode: BookingComponent.optionalGet(
              billingDetails,
              (i) => i.addresseeCode,
              userBillingDetails,
            ),
            italyEInvoicingReferencePEC: BookingComponent.optionalGet(
              billingDetails,
              (i) => i.pec,
              userBillingDetails,
            ),
            italyEInvoicingSplitPayment: BookingComponent.optionalGet(
              billingDetails,
              (i) => i.splitPayment,
              userBillingDetails,
            ),
  ...
  private buildTicketsFormGroup(
    ...
    ticketsByCategory.forEach((t) => {
      t.tickets.forEach((ticket, idx) => {
        tickets[ticket.uuid] = this.ticketService.buildFormGroupForTicket(
          ticket,
    ...
    this.reservationService
      ...
      .subscribe({
        next: (res) => {
          if (
            res.success &&
            (!res.warnings || res.warnings.length === 0 || ignoreWarnings)
          ) {
            let o: Observable<unknown> = of(true);
            if (
              this.route.snapshot.queryParamMap.has("subscription") &&
          ...
          this.globalErrors = handleServerSideValidationError(
            ...
            (key) => {
              ...
              if (result != null) {
                ...
                const links =
                  this.contactAndTicketsForm.value.additionalServices.links[
                    uuid
                  ];
      ...
      .subscribe({
        ...
        error: (err) => {
          ...
          if (errorObj != null) {
            ...
            this.feedbackService.showError(
              this.translate.instant("reservation-page.cannot-login.error"),
            );
          }
        },
      });
  ...
  getAdditionalDataForm(ticket: Ticket): FormArray | null {
    const linksGroup = <UntypedFormGroup>(
      (<UntypedFormGroup>this.contactAndTicketsForm.get("additionalServices"))
    );
  ...
  moveAdditionalService($event: MoveAdditionalServiceRequest) {
    const element = Object.assign(
      {},
      this.additionalServicesWithData[$event.currentTicketUuid][$event.index],
      {
