...
export class BookingComponent implements OnInit, AfterViewInit {
    ...
    private additionalServicesWithData: {
        [uuid: string]: AdditionalServiceWithData[];
    } = {};
    additionalServicesCount = 0;

    public static optionalGet<T>(
        billingDetails: BillingDetails,
        consumer: (b: ItalianEInvoicing) => T,
        userBillingDetails?: BillingDetails,
    ): T | null {
        const italianEInvoicing =
            billingDetails.invoicingAdditionalInfo.italianEInvoicing;
        if (italianEInvoicing != null) {
            return consumer(italianEInvoicing);
        }
        const userItalianEInvoicing =
            userBillingDetails?.invoicingAdditionalInfo?.italianEInvoicing;
    ...
    public ngOnInit(): void {
        zip(
            this.route.data,
            this.route.params,
            this.userService.authenticationStatus.pipe(first()),
        ).subscribe(([data, params, authStatus]) => {
            ...
            this.displayLoginSuggestion =
                authStatus.enabled && authStatus.user === ANONYMOUS;
            ...
            zip(
                ...
                next: ([purchaseContext, reservationInfo]) => {
                    ...
                    const invoiceRequested = purchaseContext
                        .invoicingConfiguration.onlyInvoice
                        ? true
                        : reservationInfo.invoiceRequested;
                    ...
                    if (
                        this.ticketCounts === 1 &&
                        this.purchaseContext.assignmentConfiguration
                    ...
                    const additionalServices =
                        reservationInfo.additionalServiceWithData ?? [];
                    this.contactAndTicketsForm = this.formBuilder.group({
                        firstName: this.formBuilder.control(
                            this.reservationInfo.firstName || user?.firstName,
                            [Validators.required, Validators.maxLength(255)],
                        ),
                        lastName: this.formBuilder.control(
                            this.reservationInfo.lastName || user?.lastName,
                            [Validators.required, Validators.maxLength(255)],
                        ),
                        email: this.formBuilder.control(
                            this.reservationInfo.email || user?.emailAddress,
                            [Validators.required, Validators.maxLength(255)],
                        ),
                        tickets: this.buildTicketsFormGroup(
                            this.reservationInfo.ticketsByCategory,
                            user,
                        ),
                        invoiceRequested: invoiceRequested,
                        addCompanyBillingDetails:
                            this.reservationInfo.addCompanyBillingDetails ||
                            userBillingDetails?.companyName != null,
                        billingAddressCompany:
                            billingDetails.companyName ||
                            userBillingDetails?.companyName,
                        billingAddressLine1:
                            billingDetails.addressLine1 ||
                            userBillingDetails?.addressLine1,
                        billingAddressLine2:
                            billingDetails.addressLine2 ||
                            userBillingDetails?.addressLine2,
                        billingAddressZip:
                            billingDetails.zip || userBillingDetails?.zip,
                        billingAddressCity:
                            billingDetails.city || userBillingDetails?.city,
                        billingAddressState:
                            billingDetails.state || userBillingDetails?.state,
                        vatCountryCode:
                            billingDetails.country ||
                            userBillingDetails?.country,
                        customerReference:
                            this.reservationInfo.customerReference,
                        vatNr:
                            billingDetails.taxId || userBillingDetails?.taxId,
                        skipVatNr: this.reservationInfo.skipVatNr,
                        italyEInvoicingFiscalCode: BookingComponent.optionalGet(
                            billingDetails,
                            (i) => i.fiscalCode,
                            userBillingDetails,
                        ),
                        italyEInvoicingReferenceType:
                            BookingComponent.optionalGet(
                                billingDetails,
                                (i) => i.referenceType,
                                userBillingDetails,
                            ),
                        italyEInvoicingReferenceAddresseeCode:
                            BookingComponent.optionalGet(
                                billingDetails,
                                (i) => i.addresseeCode,
                                userBillingDetails,
                        ...
                        differentSubscriptionOwner:
                            this.isDifferentOwnerDefined(),
                        subscriptionOwner: this.buildSubscriptionOwnerFormGroup(
                            this.reservationInfo.subscriptionInfos,
                            user,
                        ),
                        additionalServices:
                            this.ticketService.buildAdditionalServicesFormGroup(
                                additionalServices,
                                this.i18nService.getCurrentLang(),
                            ),
                    });

                    additionalServices.forEach((asd) => {
                        if (
                            this.additionalServicesWithData[asd.ticketUUID] !=
                            null
                        ) {
                    ...
                    setTimeout(() =>
                        this.doScroll.next(this.invoiceElement != null),
                    );

                    this.analytics.pageView(
                        purchaseContext.analyticsConfiguration,
                    );
                },
                error: (err) => {
    ...
    ngAfterViewInit(): void {
        zip(this.route.queryParams, this.doScroll.asObservable()).subscribe(
            (results) => {
                const requestInvoice: boolean = !!results[0].requestInvoice;
                if (requestInvoice && results[1]) {
                    this.contactAndTicketsForm
    ...
    private buildSubscriptionOwnerFormGroup(
        subscriptionInfos: Array<ReservationSubscriptionInfo> | undefined,
        user?: User,
    ): UntypedFormGroup {
        if (subscriptionInfos != null) {
            ...
            const email =
                ...
                (this.emailEditForbidden ? this.reservationInfo.email : null) ??
                user?.emailAddress;
            return this.formBuilder.group({
                firstName: subscriptionInfo.owner?.firstName ?? user?.firstName,
                lastName: subscriptionInfo.owner?.lastName ?? user?.lastName,
                email,
                additional: this.additionalFieldService.buildAdditionalFields(
                    subscriptionInfo.fieldConfigurationBeforeStandard,
    ...
    private buildTicketsFormGroup(
        ...
        ticketsByCategory.forEach((t) => {
            t.tickets.forEach((ticket, idx) => {
                tickets[ticket.uuid] =
                    this.ticketService.buildFormGroupForTicket(
                        ticket,
                        idx === 0 ? user : undefined,
                    );
            });
        });
    ...
    private removeUnnecessaryFields(): void {
        ...
        if (
            this.contactAndTicketsForm.get('invoiceRequested').value &&
            !this.contactAndTicketsForm.get('addCompanyBillingDetails').value
        ) {
            ['billingAddressCompany', 'vatNr', 'skipVatNr'].forEach((n) =>
                this.contactAndTicketsForm.get(n).setValue(null),
            );
        }
    }
    ...
    private validateToOverview(ignoreWarnings: boolean): void {
        this.reservationService
            .validateToOverview(
                this.reservationId,
                this.contactAndTicketsForm.value,
                this.translate.currentLang,
                ignoreWarnings,
            )
            .subscribe({
                next: (res) => {
                    if (
                        ...
                        if (
                            ...
                            o = this.reservationService.applySubscriptionCode(
                                this.reservationId,
                                subscriptionCode,
                        ...
                        firstWarning.params.forEach(
                            (v, i) => (params['' + i] = v),
                        );
                        modalRef.componentInstance.parameters = params;
                        modalRef.result.then(() =>
                            this.validateToOverview(true),
                        );
                    }
                },
                error: (err) => {
                    this.globalErrors = handleServerSideValidationError(
                        ...
                        (key) => {
                            ...
                            if (result != null) {
                                ...
                                const links =
                                    this.contactAndTicketsForm.value
                                        .additionalServices.links[uuid];
                                const index = links.findIndex(
                                    (link) => link.fields[fieldName] != null,
                                );
    ...
    cancelPendingReservation() {
        this.modalService
            .open(CancelReservationComponent, { centered: true })
            .result.then(
                (res) => {
                    if (res === 'yes') {
                        this.reservationService
    ...
    handleInvoiceRequestedChange() {
        ...
        if (
            this.contactAndTicketsForm.value.addCompanyBillingDetails === null
        ) {
            this.contactAndTicketsForm
                .get('addCompanyBillingDetails')
                .setValue(false);
        }
    }
    ...
    handleAutocomplete(fieldName: string, value: string) {
        if (this.enableAttendeeAutocomplete) {
            const ticketUUID = Object.keys(
                this.contactAndTicketsForm.get('tickets').value,
            )[0];
            const targetControl = this.contactAndTicketsForm.get(
                `tickets.${ticketUUID}.${fieldName}`,
            );
            if (
                targetControl.pristine &&
                (targetControl.value == null || targetControl.value === '')
            ) {
    ...
    getTicketForm(ticket: Ticket): UntypedFormGroup {
        return this.contactAndTicketsForm.get(
            'tickets.' + ticket.uuid,
        ) as UntypedFormGroup;
    }
                    ...
                    if (errorObj != null) {
                        ...
                        this.feedbackService.showError(
                            this.translate.instant(
                                'reservation-page.cannot-login.error',
                            ),
                        );
                    }
                },
    ...
    getAdditionalDataForm(ticket: Ticket): FormArray | null {
        const linksGroup = <UntypedFormGroup>(
            (<UntypedFormGroup>(
                this.contactAndTicketsForm.get('additionalServices')
            ))
        );
        return linksGroup.contains(ticket.uuid)
            ? <FormArray>linksGroup.get(ticket.uuid)
            : null;
    }
    ...
    get subscriptionAdditionalForm(): UntypedFormGroup {
        return this.contactAndTicketsForm
            .get('subscriptionOwner')
            .get('additional') as UntypedFormGroup;
    }

    moveAdditionalService($event: MoveAdditionalServiceRequest) {
        const element = Object.assign(
            {},
            this.additionalServicesWithData[$event.currentTicketUuid][
                $event.index
            ],
            {
                ticketUUID: $event.newTicketUuid,
            },
        );
        const additionalFormGroup = this.contactAndTicketsForm.get(
            'additionalServices',
        ) as UntypedFormGroup;
        const services = additionalFormGroup.get(
            $event.currentTicketUuid,
        ) as FormArray;
        ...
        if (additionalFormGroup.get($event.newTicketUuid) == null) {
            additionalFormGroup.addControl(
                $event.newTicketUuid,
                this.formBuilder.array([
                    this.ticketService.buildAdditionalServiceGroup(
                        element,
                        this.i18nService.getCurrentLang(),
                    ),
                ]),
            ...
            const formArray = <FormArray>(
                additionalFormGroup.get($event.newTicketUuid)
            );
            formArray.push(
                this.ticketService.buildAdditionalServiceGroup(
                    element,
        ...
        this.additionalServicesWithData[$event.currentTicketUuid] =
            this.additionalServicesWithData[$event.currentTicketUuid].filter(
    ...
    private isDifferentOwnerDefined(): boolean {
        return (
            this.reservationInfo.subscriptionInfos?.findIndex(
                (s) =>
                    s.owner != null &&
                    (s.owner.firstName != null || s.owner.lastName != null),
            ) > -1
        );
    }
}
