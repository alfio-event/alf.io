...
export class ReservationGuard {
  ...
  canActivate(
    ...
    return this.checkAndRedirect(
      route.data["type"],
      route.params[route.data["publicIdentifierParameter"]],
  ...
  private checkAndRedirect(
    ...
    return this.reservationService.getReservationStatusInfo(reservationId).pipe(
      catchError((err) =>
        of({
          status: <ReservationStatus>"NOT_FOUND",
          validatedBookingInformation: false,
        }),
      ),
      map((reservation) => {
        const selectedComponent = getCorrespondingController(
          type,
          reservation.status,
          reservation.validatedBookingInformation,
        );
        if (component === selectedComponent) {
          return true;
        }
        return this.router.createUrlTree(
          getRouteFromComponent(
            selectedComponent,
            type,
            publicIdentifier,
            reservationId,
...
function getRouteFromComponent(
  component: any,
  type: PurchaseContextType,
  publicIdentifier: string,
  reservationId: string,
...
function getCorrespondingController(
  type: PurchaseContextType,
  ...
  switch (status) {
    case "PENDING":
      return validatedBookingInformations
        ? OverviewComponent
        : BookingComponent;
    case "COMPLETE":
    case "FINALIZING":
    ...
    case "CUSTOM_OFFLINE_PAYMENT":
      return CustomOfflinePaymentComponent;
    case "DEFERRED_OFFLINE_PAYMENT":
      return DeferredOfflinePaymentComponent;
