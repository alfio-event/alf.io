...
import type {
  ...
  CustomOfflinePaymentLocalization,
} from "../../model/event";
...
export class CustomOfflinePaymentComponent implements OnInit {
  ...
  public ngOnInit(): void {
    zip(this.route.data, this.route.params).subscribe(([data, params]) => {
      ...
      zip(
        ...
        this.reservationService.getSelectedCustomPaymentMethodDetails(
          this.reservationId!,
        ),
  ...
  get translatedLocalization(): CustomOfflinePaymentLocalization | null {
    ...
    const localizationKeys = Object.keys(
      this.customPaymentMethodDetails.localizations,
    );
    let translatedLocalization =
      this.customPaymentMethodDetails?.localizations["en"] ||
      this.customPaymentMethodDetails?.localizations[localizationKeys[0]];

    if (localizationKeys.includes(currentLang)) {
      translatedLocalization =
        this.customPaymentMethodDetails.localizations[currentLang];
    }
