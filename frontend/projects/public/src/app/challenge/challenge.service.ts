...
export class ChallengeService {
    ...
    requestChallenge(): Observable<string> {
        return this.infoService.getInfo().pipe(
            mergeMap((info) => {
                const modalRef = this.modalService.open(ChallengeComponent, {
                    centered: true,
                    backdrop: 'static',
                });
                modalRef.componentInstance.challengeConfiguration =
                    info.challengeConfiguration;
                return (
                    from(modalRef.result)
                        ...
                        .pipe(
                            ...
                            catchError((err) =>
                                throwError(() => new ChallengeError(err)),
                            ),
                        )
                );
            }),
    ...
    public static isChallengeError(err: any): boolean {
        return (
            ...
            (err instanceof HttpErrorResponse &&
                err.status === 403 &&
                (err.headers.has('Alfio-Verification-Missing') ||
                    err.headers.get('cf-mitigated') === 'challenge'))
        );
    }
}
