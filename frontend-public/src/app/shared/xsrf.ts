...
export class DOMXsrfTokenExtractor implements HttpXsrfTokenExtractor {
  ...
  getToken(): string | null {
    ...
    return this.doc.head.querySelector<HTMLMetaElement>('meta[name="XSRF_TOKEN"]')?.content;
  }

  updateToken(newVal: string | null): void {
    if (newVal === this.lastToken) {
      return;
    }
...
export class DOMGidExtractor {
  ...
  getToken(): string | null {
    ...
    return this.doc.head.querySelector<HTMLMetaElement>('meta[name="GID"]')?.content;
  }
...
export class AuthTokenInterceptor implements HttpInterceptor {
  ...
  private domGidExtractor: DOMGidExtractor | null;
  private domXsrfTokenExtractor: DOMXsrfTokenExtractor | null;
