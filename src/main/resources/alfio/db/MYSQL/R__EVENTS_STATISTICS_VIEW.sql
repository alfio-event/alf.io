...
drop view if exists ticket_categories_statistics_pre;
drop view if exists ticket_category_statistics;

drop view if exists tickets_stats_view;
drop view if exists tickets_stuck_view;

drop view if exists ticket_category_metainfo_view;

create view ticket_category_metainfo_view as (select max_tickets, bounded, id, event_id, expiration < now() as is_expired from ticket_category);

create view tickets_stats_view as (
select
...
create view tickets_stuck_view as (
...
create view ticket_categories_statistics_pre as (
...
from
 ticket_category_metainfo_view ticket_cat
left join tickets_stats_view on ticket_cat.id = tickets_stats_view.category_id
left join tickets_stuck_view on ticket_cat.id = tickets_stuck_view.category_id
...
from ticket_categories_statistics_pre res);
