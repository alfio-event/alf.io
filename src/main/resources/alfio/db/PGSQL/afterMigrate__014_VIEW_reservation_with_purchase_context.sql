...
create view reservation_with_purchase_context as (
    ...
    select res.*, 'event', e.short_name as pc_public_identifier, jsonb_build_object('en', e.display_name) as pc_title from reservations res join event e on e.id = res.tr_event_id_fk
    union
    select res.*, 'subscription', sd.id::text as public_identifier, sd.title  as pc_title from reservations res
