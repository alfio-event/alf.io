...
public interface OrganizationRepository {
    ...
    @Query("""
           INSERT INTO organization(name, description, email, name_openid, slug)\
    ...
    @Query("""
        select * from ((select organization.* from organization inner join j_user_organization on org_id = organization.id where j_user_organization.user_id = (select ba_user.id from ba_user where ba_user.username = :username)) \
         union \
    ...
    @Query("""
        (select organization.* from organization inner join j_user_organization on org_id = organization.id where j_user_organization.user_id = (select ba_user.id from ba_user where ba_user.username = :username) and organization.id = :orgId) \
         union \
