...
public interface ExtensionRepository {
    ...
    @Query("""
        insert into extension_support(path, name, display_name, hash, enabled, async, script, metadata) values \
    ...
    @Query("""
        update extension_support set display_name = :displayName,\
    ...
    @Query("""
        insert into extension_event(es_id_fk, event) values \
         (:extensionId, :event)\
        """)
    ...
    @Query("""
        select a3.es_id, a3.path, a3.name, a3.hash from \
         (select a1.* from \
    ...
    @Query("""
        insert into extension_configuration_metadata(ecm_es_id_fk, ecm_name, ecm_description, ecm_type, ecm_configuration_level, ecm_mandatory) \
    ...
    @Query("""
        select ecm_id, ecm_name, ecm_configuration_level, ecm_description, ecm_type, ecm_mandatory, path, es_id, name, display_name, conf_path, conf_value\
    ...
    @Query("""
        select ecm_name, conf_value from \
    ...
    @Query("""
        select distinct a3.capability, a3.capability_detail from \
