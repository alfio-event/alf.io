...
public interface SubscriptionRepository {
    ...
    String INSERT_SUBSCRIPTION = """
        insert into subscription(id, subscription_descriptor_fk, reservation_id_fk, max_usage, \
    ...
    @Query("""
        insert into subscription_descriptor (\
    ...
    @Query("""
        update subscription_descriptor set title = :title::jsonb, description = :description::jsonb, max_available = :maxAvailable,\
    ...
    @Query("""
        select * from subscription_descriptor where organization_id_fk = :organizationId\
         and status = 'ACTIVE'\
    ...
    @Query("""
        select subscription_descriptor.* from subscription_descriptor\
    ...
    @Query("""
        select * from subscription_descriptor where status = 'ACTIVE' and organization_id_fk = :organizationId\
    ...
    @Query("""
        delete from subscription_event where event_id_fk = :eventId and organization_id_fk = :organizationId\
    ...
    @Query("""
        update subscription set reservation_id_fk = :reservationId, status = :status::allocation_status,\
    ...
    @Query("""
        update subscription set status = :status::allocation_status, first_name = :firstName, last_name = :lastName,\
    ...
    @Query("""
        update subscription set first_name = :firstName, last_name = :lastName, email_address = :email,\
    ...
    @Query("""
        update subscription set status = 'CANCELLED' where reservation_id_fk = :reservationId\
