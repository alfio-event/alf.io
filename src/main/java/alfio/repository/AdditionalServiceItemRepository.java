...
public interface AdditionalServiceItemRepository {
    ...
    String SELECT_BOOKED_ADDITIONAL_SERVICES = """
        select asd.value as as_name, ads.id as_id, count(ads.id) as qty from additional_service_item ai\
    ...
    @Query("""
        select count(*) > 0 from additional_service_item \
    ...
    @Query(type = QueryType.TEMPLATE, value = """
        insert into additional_service_item (uuid, creation, tickets_reservation_uuid, additional_service_id_fk, status, event_id_fk, src_price_cts, final_price_cts, vat_cts, discount_cts, currency_code) \
    ...
    @Query(type = QueryType.TEMPLATE, value = """
        update additional_service_item set tickets_reservation_uuid = :ticketsReservationUuid, status = :status, src_price_cts = :srcPriceCts, final_price_cts = :finalPriceCts, vat_cts = :vatCts, discount_cts = :discountCts, currency_code = :currencyCode\
    ...
    @Query(type = QueryType.TEMPLATE, value = """
        update additional_service_item set ticket_id_fk = :ticketId\
