...
public interface EmailMessageRepository {
    ...
    @Query("select id, event_id, status, recipient, subject, message, attachments, checksum, request_ts, sent_ts from email_message where status = 'WAITING' limit :max for update")
    List<EmailMessage> loadWaitingForProcessing(@Bind("max") int max);

    @Query("select id, event_id, status, recipient, subject, message, attachments, checksum, request_ts, sent_ts from email_message where owner = :owner")
    List<EmailMessage> loadForProcessing(@Bind("owner") String owner);
