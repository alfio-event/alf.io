...
public class MollieManager {
	...
	public String createCheckoutRequest(Event event, String reservationId,
		...
		Map<String, String> initialMetadata = new HashMap<>();
		initialMetadata.put("reservationId", reservationId);
		initialMetadata.put("email", customerInformation.getEmail());
		initialMetadata.put("fullName", customerInformation.getFullName());
		if (StringUtils.isNotBlank(customerInformation.getBillingAddress())) {
			initialMetadata.put("billingAddress", customerInformation.getBillingAddress());
		}
