/**
 * This file is part of alf.io.
 *
 * alf.io is free software: you can redistribute it and/or modify
 * it under the terms of the GNU General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * alf.io is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 *
 * You should have received a copy of the GNU General Public License
 * along with alf.io.  If not, see <http://www.gnu.org/licenses/>.
 */
package alfio.model.modification;

import alfio.model.PriceContainer;
import alfio.model.SubscriptionDescriptor.SubscriptionAvailability;
import alfio.util.MonetaryUtil;
import com.fasterxml.jackson.annotation.JsonProperty;
import lombok.Getter;

import java.math.BigDecimal;
import java.time.ZonedDateTime;
import java.util.Map;

@Getter
public class SubscriptionDescriptorModification {
    private final Long id;
    private final Integer maxEntries;
    private final ZonedDateTime validFrom;
    private final ZonedDateTime validTo;
    private final BigDecimal price;
    private final BigDecimal vat;
    private final PriceContainer.VatStatus vatStatus;
    private final String currency;
    private final SubscriptionAvailability availability;
    private final Boolean isPublic;
    private final Map<String, String> title;
    private final Map<String, String> description;
    private final int organizationId;

    public SubscriptionDescriptorModification(@JsonProperty("id") Long id,
                                              @JsonProperty("maxEntries") Integer maxEntries,
                                              @JsonProperty("validFrom") ZonedDateTime validFrom,
                                              @JsonProperty("validTo") ZonedDateTime validTo,
                                              @JsonProperty("price") BigDecimal price,
                                              @JsonProperty("vat") BigDecimal vat,
                                              @JsonProperty("vatStatus") PriceContainer.VatStatus vatStatus,
                                              @JsonProperty("currency") String currency,
                                              @JsonProperty("availability") SubscriptionAvailability availability,
                                              @JsonProperty("isPublic") Boolean isPublic,
                                              @JsonProperty("title") Map<String, String> title,
                                              @JsonProperty("description") Map<String, String> description,
                                              @JsonProperty("organizationId")  int organizationId) {
        this.id = id;
        this.maxEntries = maxEntries;
        this.validFrom = validFrom;
        this.validTo = validTo;
        this.price = price;
        this.vat = vat;
        this.vatStatus = vatStatus;
        this.currency = currency;
        this.availability = availability;
        this.isPublic = isPublic;
        this.title = title;
        this.description = description;
        this.organizationId = organizationId;
    }

    public int getPriceCts() {
        return MonetaryUtil.unitToCents(price, currency);
    }
}
