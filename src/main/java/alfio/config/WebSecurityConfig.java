    ...
    public static class FormBasedWebSecurity extends WebSecurityConfigurerAdapter {
        ...
        protected void configure(HttpSecurity http) throws Exception {
            ...
            configurer.csrfTokenRepository(csrfTokenRepository)
                ...
                .antMatchers(HttpMethod.POST, ADMIN_API + "/users/current/edit", ADMIN_API + "/users/current/update-password").hasAnyRole(ADMIN, OWNER, SUPERVISOR)
                .antMatchers(HttpMethod.POST, ADMIN_API + "/users/check", ADMIN_API + "/users/edit", ADMIN_API + "/users/update-password").hasRole(ADMIN)
