...
public interface TicketCategoryRepository {
    ...
    @Query("insert into ticket_category(inception, expiration, name, description, max_tickets, price_cts, access_restricted, tc_status) " +
            "values(:inception, :expiration, :name, :description, :max_tickets, :price, :accessRestricted, 'ACTIVE')")
    @AutoGeneratedKey("id")
    ...
    @Query("select * from ticket_category inner join j_event_ticket_category on ticket_category_id = id " +
            "where event_id = :eventId  and tc_status = 'ACTIVE' order by inception asc, expiration asc, id asc")
